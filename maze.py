...
class Maze(object):
    ...
    def __init__(self, num_rows, num_cols, id):
        ...
        try:
            for line in solution:
                if line == 'felvesz\n':
                    ...
                    x = int(line.split(" ")[0])
                    y = int(line.split(" ")[1])
                    if x >= 0 and x <= self.num_rows and y >= 0 and y <= self.num_cols and abs(x - curr_poz.row)<=1 and abs(y - curr_poz.col)<=1:
                        #ide eljut ha szabályos a lépés
                        if x == curr_poz.row -1 and y == curr_poz.col:      #fel
